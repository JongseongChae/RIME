...
def discr_update_generator(obs, action_policy, action_expert, num_steps, num_processes,
    ...
    for indices in sampler:
        obs_batch = obs[:-1].view(-1, obs.size(-1))[indices]
        actions_policy_batch = action_policy.view(-1, action_policy.size(-1))[indices]
        actions_expert_batch = action_expert.view(-1, action_expert.size(-1))[indices]

        yield obs_batch, actions_policy_batch, actions_expert_batch
...
class RolloutStorage(object):
    ...
    def extended_feed_forward_generator(self,
                               advantages,
                               num_mini_batch=None,
                               mini_batch_size=None):
        num_steps, num_processes = self.rewards.size()[0:2]
        ...
        for indices in sampler:
            ...
            yield obs_batch, recurrent_hidden_states_batch, actions_batch, \
                value_preds_batch, return_batch, masks_batch, old_action_log_probs_batch, adv_targ, next_obs_batch
