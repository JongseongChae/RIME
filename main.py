...
if __name__ == '__main__':
    ...
    if (args.sampled_envs == 2) or (args.sampled_envs == 3):
        def classify_data1(seeds, data, index, store):
            for i in range(len(seeds)):
                store.append(data[i][1][index])

        def classify_data0(seeds, data, store):
            for i in range(len(seeds)):
        ...
        classify_data0(seeds, data, record_perf)
        classify_data1(seeds, data, 0, timesteps)
        classify_data1(seeds, data, 1, learning_curve1)
        if args.sampled_envs == 2:
            classify_data1(seeds, data, 2, learning_curve2)
        elif args.sampled_envs == 3:
            classify_data1(seeds, data, 2, learning_curve2)
            classify_data1(seeds, data, 3, learning_curve3)
        ...
        t = np.mean(timesteps, axis=0)
        avg_learning_curve1 = np.mean(learning_curve1, axis=0)
        std_learning_curve1 = np.std(learning_curve1, axis=0)
        if args.sampled_envs == 2:
            avg_learning_curve2 = np.mean(learning_curve2, axis=0)
            std_learning_curve2 = np.std(learning_curve2, axis=0)
        elif args.sampled_envs == 3:
            avg_learning_curve2 = np.mean(learning_curve2, axis=0)
            std_learning_curve2 = np.std(learning_curve2, axis=0)
            avg_learning_curve3 = np.mean(learning_curve3, axis=0)
            std_learning_curve3 = np.std(learning_curve3, axis=0)

        rec_save_path = 'results/' + str(data[0][2])

        if args.sampled_envs == 2:
            np.savez(rec_save_path, record_perf=[avg_record_perf, std_record_perf],
                     learning_curve1=[avg_learning_curve1, std_learning_curve1],
                     learning_curve2=[avg_learning_curve2, std_learning_curve2], allow_pickle=True)
        elif args.sampled_envs == 3:
            np.savez(rec_save_path, record_perf=[avg_record_perf, std_record_perf],
                     learning_curve1=[avg_learning_curve1, std_learning_curve1],
                     learning_curve2=[avg_learning_curve2, std_learning_curve2],
                     learning_curve3=[avg_learning_curve3, std_learning_curve3], allow_pickle=True)
        ...
        plt.figure()
        plt.plot(t, avg_learning_curve1, label='true_return_env1')
        plt.fill_between(t, avg_learning_curve1 - std_learning_curve1, avg_learning_curve1 + std_learning_curve1, alpha=0.1)
        if args.sampled_envs == 2:
            plt.plot(t, avg_learning_curve2, label='true_return_env2')
            plt.fill_between(t, avg_learning_curve2 - std_learning_curve2, avg_learning_curve2 + std_learning_curve2, alpha=0.1)
        elif args.sampled_envs == 3:
            plt.plot(t, avg_learning_curve2, label='true_return_env2')
            plt.fill_between(t, avg_learning_curve2 - std_learning_curve2, avg_learning_curve2 + std_learning_curve2, alpha=0.1)
            plt.plot(t, avg_learning_curve3, label='true_return_env3')
            plt.fill_between(t, avg_learning_curve3 - std_learning_curve3, avg_learning_curve3 + std_learning_curve3, alpha=0.1)
        plt.legend()
        plt.savefig(rec_save_path + '_learning_curve.png')
        ...
        def classify_data_j(seeds, data, index, j, store):
            for i in range(len(seeds)):
                store.append(data[i][j][index])

        def classify_data0(seeds, data, index, store):
            for i in range(len(seeds)):
                store.append(data[i][index])

        classify_data0(seeds, data, 0, record_perf)
        classify_data_j(seeds, data, 0, 1, timesteps)
        classify_data_j(seeds, data, 1, 1, learning_curve1)
        classify_data_j(seeds, data, 2, 1, learning_curve2)
        classify_data_j(seeds, data, 3, 1, learning_curve3)
        classify_data_j(seeds, data, 4, 1, learning_curve4)
        ...
        avg_learning_curve3 = np.mean(learning_curve3, axis=0)
        std_learning_curve3 = np.std(learning_curve3, axis=0)
        avg_learning_curve4 = np.mean(learning_curve4, axis=0)
        std_learning_curve4 = np.std(learning_curve4, axis=0)

        rec_save_path = 'results/' + str(data[0][-1])  # data[2] is excute_name

        np.savez(rec_save_path, record_perf_2par_gm=[avg_record_perf_2par_gm, std_record_perf_2par_gm],
                 learning_curve1=[avg_learning_curve1, std_learning_curve1],
                 learning_curve2=[avg_learning_curve2, std_learning_curve2],
                 learning_curve3=[avg_learning_curve3, std_learning_curve3],
                 learning_curve4=[avg_learning_curve4, std_learning_curve4],
                 allow_pickle=True)
        ...
        plt.colorbar()
        plt.tight_layout()
        plt.savefig(rec_save_path + '_perf_2par_gm.png')
        ...
        plt.plot(t, avg_learning_curve3, label='true_return_env3')
        plt.fill_between(t, avg_learning_curve3 - std_learning_curve3, avg_learning_curve3 + std_learning_curve3, alpha=0.1)
        plt.plot(t, avg_learning_curve4, label='true_return_env4')
        plt.fill_between(t, avg_learning_curve4 - std_learning_curve4, avg_learning_curve4 + std_learning_curve4, alpha=0.1)
        plt.legend(fontsize=8)
        plt.savefig(rec_save_path + '_perf_learning.png')
